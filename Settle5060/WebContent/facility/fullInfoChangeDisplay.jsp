...
<p>価格範囲：<label>最低価格<input type="number" name="low_price" id="low_price" value="${fac_info.low_price}" max="" min=0 required></label>～<label>最高価格<input type="number" name="high_price" id="high_price" value="${fac_info.high_price}" min="" required></label></p>
<p>初期価格：<label>初期価格<input type="number" name="init_price" id="init_price" value="${fac_info.init_price}" required></label></p>
<p>当日券価格：<label>当日券価格<input type="number" name="sd_tkt_price" value="${fac_info.sd_tkt_price}" required></label></p>
<p>子供割引：<label>割引率<input type="number" name="chld_dsc" max=100 min=0 value="${fac_info.chld_dsc}" required></label>%引き</p>
...
window.onload = function(){
	...
	document.getElementById("high_price").min = ${fac_info.low_price}
}
 function priceSetting(){
	const low_price = document.getElementById("low_price").value;
	const high_price = document.getElementById("high_price").value;
	document.getElementById("low_price").max = high_price;
	document.getElementById("high_price").min = low_price;
	document.getElementById("init_price").min = low_price;
	document.getElementById("init_price").max = high_price;
